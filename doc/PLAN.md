...
| TL-C → CHI bridge variant — **Stages 1–3 of 7 landed 2026-05-29**.  `TLCToCHI` module targeting CHI Issue-E, RN-F upstream face.  Acquire engine handles all four cold/upgrade cases — `AcquireBlock(NtoB)` → `ReadShared` (data, toB), `AcquireBlock(NtoT)` → `ReadUnique` (data, toT), `AcquireBlock(BtoT)` → `MakeUnique` (no data, toT), `AcquirePerm(*)` → `MakeUnique` (no data, toT) — each followed by TL-E `GrantAck` → CHI `CompAck`.  **Verification parity:** `make lint-chi` (0 warnings), `make sim-chi` (4 directed jobs PASS via a hand-rolled CHI HN model in `test/cpp/tb_chi.cpp`), `make formal-chi` BMC depth 20 PASS with F-CHI-1..3 plus both cover goals reached, `make cocotb-chi` (1 mixed-acquire test PASS on Icarus).  Now in `make regress` and `regress-chi`.  Stages 4–7 (release, snoop, atomics/CMO, full verification + CI parity) gated on a separate go-ahead — see [`doc/CHI_PLAN.md`](CHI_PLAN.md). |

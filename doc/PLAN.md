...
| TL-C → CHI bridge variant — **Stages 1–4 of 7 landed 2026-05-29**.  `TLCToCHI` module targeting CHI Issue-E, RN-F upstream face.  **Acquire engine** covers all four cold/upgrade cases (NtoB→ReadShared, NtoT→ReadUnique, BtoT→MakeUnique, AcquirePerm→MakeUnique) with TL-E `GrantAck` → CHI `CompAck`.  **Release engine** runs in parallel and covers Release→Evict and ReleaseData→WriteBackFull/WriteCleanFull, with CompDBIDResp/CopyBackWrData handshake on the writeback path.  TxnID partition (acquire MSB=0, release MSB=1) keeps the two engines disjoint on shared CHI channels.  **Verification parity:** `make lint-chi` (0 warnings), `make sim-chi` (8 directed jobs PASS: 4 acquire + 4 release), `make formal-chi` BMC depth 20 PASS with F-CHI-1..5 plus 6 cover goals reached, `make cocotb-chi` (2 mixed tests PASS on Icarus).  Now in `make regress` and `regress-chi`.  Stages 5–7 (snoop, atomics/CMO, full verification + CI parity) gated on a separate go-ahead — see [`doc/CHI_PLAN.md`](CHI_PLAN.md). |

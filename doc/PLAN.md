...
| ~~TL-UL → AXI4-Lite bridge variant~~ ✓ DONE 2026-05-28 — new `TLULToAXILite` module in `src/main/scala/tlbridge/TLULToAXILite.scala`, parameterized by `ULBridgeParams` (default `dataBits=32`, also valid at 64).  Single-beat, three engines (read, write, hint) + local-error slot; AW/W issued in parallel.  Emitted to `generated/ulite/`.  Full verification parity with the TL-UH bridge: `make lint-ulite` (0 warnings), `make sim-ulite` (24 jobs PASS, peakConcurrency=3), `make formal-ulite` (BMC depth 20 + 4 cover witnesses PASS), `make cocotb-ulite` (5/5 PASS on Icarus).  Spec at `doc/DESIGN_SPEC_ULITE.md`; property catalog appended as §7 of `doc/ASSERTIONS.md`.  **Follow-up:** randomized C++ sweep (currently directed-only); width sweep for 64-bit AXI-Lite. |
| ~~TL-UC (via TL-C) → AXI4 bridge variant~~ ✓ DONE 2026-05-28 — new `TLUCToAXI4` module in `src/main/scala/tlbridge/TLUCToAXI4.scala`.  Carries TL-C wire shape (A + B + C + D + E) with no coherence; TL-B tied off, always grants Tip on Acquire, accepts `Release`/`ReleaseData` on TL-C and forwards dirty writebacks to AXI4 via the same AW/W path the write engine uses.  Six parallel engines (read, write, hint, atomic, acquire, release) + local-error slot.  Emitted to `generated/uc/`.  Full verification parity: `make lint-uc` (0 warnings), `make sim-uc` (11 jobs PASS, peakConcurrency=2), `make formal-uc` (BMC depth 30 + 5 cover witnesses), `make cocotb-uc` (6/6 PASS on Icarus).  Spec at `doc/DESIGN_SPEC_UC.md`; property catalog appended as §8 of `doc/ASSERTIONS.md`.  Serves as stepping stone for the TL-C → CHI bridge planned below (same channel plumbing). |
| ~~TL-C → CHI bridge variant~~ ✓ **All 7 stages landed 2026-05-29**.  `TLCToCHI` module targeting CHI Issue-E, RN-F upstream face.  Four engines run in parallel.  **Acquire** covers the four cold/upgrade cases (NtoB→ReadShared, NtoT→ReadUnique, BtoT→MakeUnique, AcquirePerm→MakeUnique) with TL-E `GrantAck` → CHI `CompAck`.  **Release** covers Release→Evict and ReleaseData→WriteBackFull/WriteCleanFull (CompDBIDResp/CopyBackWrData handshake).  **Snoop** (Stage 5) turns CHI `Snp*` into TL-B `Probe` and feeds TL-C `ProbeAck`/`ProbeAckData` back as CHI `SnpResp`/`SnpRespData`.  **Uncached** (Stage 6) handles the TL-A uncached space: Get→ReadOnce, Put→WriteUnique*, Hint(prefetch)→CleanShared/CleanInvalid, Arithmetic/Logical→AtomicLoad*/AtomicSwap (with NonCopyBackWrData operand and pre-op CompData return; AND→Clr with inverted operand).  A 2-bit txnID partition (acquire 00 / release 10 / uncached 01) keeps the request engines disjoint; shared CHI channels and TL-D are arbitrated acquire/release-first.  **Full verification surface (Stage 7):** `make lint-chi` (0 warnings), `make sim-chi` (28 directed + 120 randomized jobs, seed `0xC0FFEE`), `make formal-chi` BMC depth 30 PASS with F-CHI-1..11 plus 19 cover goals reached, `make cocotb-chi` (4 tests on Icarus), `make coverage-chi` (80.3 % line; remainder is unused Issue-E sideband ports — see ASSERTIONS §9.7).  In `make regress`/`regress-chi`/`make ci` and a dedicated `chi` GitHub Actions job.  Catalog at `doc/ASSERTIONS.md` §9.  **Known gaps (non-blocking, ASSERTIONS §10):** probe↔release race not collapsed (F-CHI-8 proves no corruption); formal scopes TL-A to single-beat (multi-beat Put in sim/cocotb); no CAS; HN injects no errors so `denied` is unexercised.  See [`doc/CHI_PLAN.md`](CHI_PLAN.md). |

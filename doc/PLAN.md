...
| TL-C → CHI bridge variant — **Stages 1–5 of 7 landed 2026-05-29**.  `TLCToCHI` module targeting CHI Issue-E, RN-F upstream face.  **Acquire engine** covers all four cold/upgrade cases (NtoB→ReadShared, NtoT→ReadUnique, BtoT→MakeUnique, AcquirePerm→MakeUnique) with TL-E `GrantAck` → CHI `CompAck`.  **Release engine** runs in parallel and covers Release→Evict and ReleaseData→WriteBackFull/WriteCleanFull, with CompDBIDResp/CopyBackWrData handshake on the writeback path.  **Snoop engine** (Stage 5) turns CHI `Snp*` into TL-B `Probe` and feeds TL-C `ProbeAck`/`ProbeAckData` back as CHI `SnpResp`/`SnpRespData`; runs in parallel with the other two, sharing txrsp/txdat under acquire/release-first priority, with C-channel intake opcode-routed so each beat lands on exactly one engine.  TxnID partition (acquire MSB=0, release MSB=1) keeps the request engines disjoint on shared CHI channels.  **Verification parity:** `make lint-chi` (0 warnings), `make sim-chi` (12 directed jobs PASS: 4 acquire + 4 release + 4 snoop), `make formal-chi` BMC depth 20 PASS with F-CHI-1..8 plus 11 cover goals reached (target now runs both `chi-bmc` and `chi-cover`), `make cocotb-chi` (3 mixed tests PASS on Icarus).  Now in `make regress` and `regress-chi`.  **Stage 5 limit:** the probe ↔ release race is not collapsed (engines complete independently; F-CHI-8 proves no cross-engine corruption).  Stages 6–7 (atomics/CMO, full verification + CI parity) gated on a separate go-ahead — see [`doc/CHI_PLAN.md`](CHI_PLAN.md). |

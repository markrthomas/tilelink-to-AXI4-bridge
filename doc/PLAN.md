...
| TL-C → CHI bridge variant — **Stages 1–6 of 7 landed 2026-05-29**.  `TLCToCHI` module targeting CHI Issue-E, RN-F upstream face.  Four engines run in parallel.  **Acquire** covers the four cold/upgrade cases (NtoB→ReadShared, NtoT→ReadUnique, BtoT→MakeUnique, AcquirePerm→MakeUnique) with TL-E `GrantAck` → CHI `CompAck`.  **Release** covers Release→Evict and ReleaseData→WriteBackFull/WriteCleanFull (CompDBIDResp/CopyBackWrData handshake).  **Snoop** (Stage 5) turns CHI `Snp*` into TL-B `Probe` and feeds TL-C `ProbeAck`/`ProbeAckData` back as CHI `SnpResp`/`SnpRespData`.  **Uncached** (Stage 6) handles the TL-A uncached space: Get→ReadOnce, Put→WriteUnique*, Hint(prefetch)→CleanShared/CleanInvalid, Arithmetic/Logical→AtomicLoad*/AtomicSwap (with NonCopyBackWrData operand and pre-op CompData return; AND→Clr with inverted operand).  A 2-bit txnID partition (acquire 00 / release 10 / uncached 01) keeps the request engines disjoint; shared CHI channels and TL-D are arbitrated acquire/release-first.  **Verification parity:** `make lint-chi` (0 warnings), `make sim-chi` (23 directed jobs PASS: 4 acquire + 4 release + 4 snoop + 11 uncached/atomic), `make formal-chi` BMC depth 20 PASS with F-CHI-1..11 plus 19 cover goals reached (target runs both `chi-bmc` and `chi-cover`), `make cocotb-chi` (4 mixed tests PASS on Icarus).  Now in `make regress` and `regress-chi`.  **Limits:** probe↔release race not collapsed (F-CHI-8 proves no corruption); formal scopes TL-A to single-beat (multi-beat Put in sim/cocotb); no CAS.  Stage 7 (full verification + CI parity) gated on a separate go-ahead — see [`doc/CHI_PLAN.md`](CHI_PLAN.md). |

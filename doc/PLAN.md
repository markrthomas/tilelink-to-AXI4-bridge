...
| TL-C → CHI bridge variant — **Stage 1 of 7 landed 2026-05-28**. New sibling `TLCToCHI` module targeting CHI Issue-E.  Direction: TL-C upstream (RN-F) → CHI downstream.  Full TL-C parity is the eventual scope; work staged into 7 milestones — see [`doc/CHI_PLAN.md`](CHI_PLAN.md).  **Stage 1 deliverables (done):** `doc/DESIGN_SPEC_CHI.md` (full mapping doc), `src/main/scala/tlbridge/CHIBundles.scala` (Issue-E REQ/RSP/DAT/SNP bundles + opcode constants), `src/main/scala/tlbridge/TLCToCHI.scala` (module skeleton with all 9 channels wired and tied to safe defaults), Makefile/cocotb/formal scaffolding (`lint-chi` clean, in `make regress`).  **No functional behavior yet** — Stage 2 (read-shared path) is gated on a separate go-ahead.  Four open questions from §2 of `CHI_PLAN.md` all settled (DCT/DMT off, hand-rolled HN, single-line atomics, 64 B line). |

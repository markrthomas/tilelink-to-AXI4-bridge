...
| TL-C → CHI bridge variant — **planned, not started** 2026-05-28. New sibling `TLCToCHI` module targeting CHI Issue-E.  Direction: TL-C upstream (RN-F) → CHI downstream.  Full TL-C parity (all 5 channels + atomics + CMO + prefetch) is the eventual scope; work is broken into 7 stages of independently shippable milestones — see [`doc/CHI_PLAN.md`](CHI_PLAN.md).  Will integrate as a third Makefile variant (`lint-chi`, `sim-chi`, `formal-chi`, `cocotb-chi`) alongside the existing two bridges.  [`doc/TLC_EVALUATION.md`](TLC_EVALUATION.md) reopened with a status note clarifying that the AXI4 bridges stay on TL-UH; the CHI bridge is a separate path.  **Gated on:** answers to the four open questions in §2 of `CHI_PLAN.md` and an explicit go-ahead to start Stage 1 (mapping doc + skeleton). |
